# Embedded file name: vendor\httplib2\__init__.py
...
class HTTPSConnectionWithTimeout(httplib.HTTPSConnection):
    ...
    def connect(self):
        ...
        msg = 'getaddrinfo returns an empty list'
        for family, socktype, proto, canonname, sockaddr in socket.getaddrinfo(self.host, self.port, 0, socket.SOCK_STREAM):
            try:
                if self.proxy_info and self.proxy_info.isgood():
...
try:
    ...
    class AppEngineHttpConnection(object):
        ...
        def __init__(self, host, port = None, key_file = None, cert_file = None, strict = None, timeout = None, proxy_info = None, ca_certs = None, disable_certificate_validation = False):
            self.host = host
            self.port = port
            self.timeout = timeout
    ...
    class AppEngineHttpsConnection(AppEngineHttpConnection):
        ...
        def __init__(self, host, port = None, key_file = None, cert_file = None, strict = None, timeout = None, proxy_info = None):
            AppEngineHttpConnection.__init__(self, host, port, key_file, cert_file, strict, timeout, proxy_info)
            self.scheme = 'https'
        ...
        try:
            ...
            if conn_key in self.connections:
                ...
                if issubclass(connection_type, HTTPSConnectionWithTimeout):
                    if certs:
                        conn = self.connections[conn_key] = connection_type(authority, key_file=certs[0][0], cert_file=certs[0][1], timeout=self.timeout, proxy_info=self.proxy_info, ca_certs=self.ca_certs, disable_ssl_certificate_validation=self.disable_ssl_certificate_validation)
                    else:
                        conn = self.connections[conn_key] = connection_type(authority, timeout=self.timeout, proxy_info=self.proxy_info, ca_certs=self.ca_certs, disable_ssl_certificate_validation=self.disable_ssl_certificate_validation)
                else:
